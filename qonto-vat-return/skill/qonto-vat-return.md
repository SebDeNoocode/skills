...
- **On receipts**: match the period's **incoming transactions** to client invoices (amount + counterparty + reference; `paid_at` and `mark_client_invoice_as_paid` status help) and take the matched invoices' VAT breakdown. Incoming credits with no matching invoice → listed separately as "unassigned receipts", never silently taxed or ignored.
